...
export function getDateRange(
  ...
  const earliestTime = dates.reduce(
    (min, date) => (date < min ? date : min),
    dates[0] ?? Date.now()
  )
  const latestTime = dates.reduce((max, date) => (date > max ? date : max), dates[0] ?? Date.now())
...
/**
 * Calculates the average closeness of competition across all rounds.
 * Closeness is measured by the standard deviation of points received within each round.
 * Lower values indicate closer competition (points are more evenly distributed).
 *
 * @param performancesByRound - Map of round IDs to arrays of performances in that round
 * @returns Average standard deviation across all rounds, or 0 if no rounds or insufficient data
 */
export function calculateAvgCloseness(
  ...
  return stddevs.reduce((sum, val) => sum + val, 0) / stddevs.length
}
